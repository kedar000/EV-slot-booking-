  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 bg-white dark:bg-gray-800 shadow-md transition-colors duration-300">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        >
          <EVSlotLogo width={120} height={40} isDarkMode={isDarkMode} />
        </motion.div>
        <nav className="hidden md:flex space-x-4">
          {navItems.map((item) => (
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              animate={activeSection === item.href.slice(1) ? { scale: 1.1 } : { scale: 1 }}
            >
              {item.name}
            </motion.a>
          ))}
          ...
          <AnimatePresence mode="wait">
            {isLoggedIn ? (
              <motion.button
                ...
                className="flex items-center space-x-2 px-4 py-2 rounded-full bg-gradient-to-r from-red-500 to-orange-500 text-white hover:from-red-600 hover:to-orange-600 transition-colors duration-300"
              ...
              <motion.button
                key="login"
                onClick={handleLogin}
                className="flex items-center space-x-2 px-4 py-2 rounded-full bg-gradient-to-r from-green-500 to-teal-500 text-white hover:from-green-600 hover:to-teal-600 transition-colors duration-300"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
            ...
            {navItems.map((item, index) => (
              <motion.a
                ...
                className={`block px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-300 ${
                  activeSection === item.href.slice(1) ? 'text-green-500 dark:text-green-400 font-bold' : ''
                }`}
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
